...
class _DrugSearchScreenState extends State<DrugSearchScreen>
  ...
  // يمنع تشغيل أكثر من Refresh في نفس الوقت
  bool _refreshing = false;

  // لمعرفة حالة التطبيق السابقة
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    // نعمل Refresh فقط عند الانتقال فعليًا من
    // background/inactive إلى resumed
  ...
  void didChangeDependencies() {
    ...
    // لا يوجد Refresh هنا.
    //
    // الـ Refresh يتم من lifecycle فقط،
    // حتى لا نحصل على Refresh مزدوج.
  }
  ...
  Future<void> loadDrugs() async {
    try {
      ...
      setState(() {
        allDrugs = List<DrugModel>.from(data);
        loading = false;
      });
  ...
  Future<void> refreshDrugs() async {
    ...
    if (_refreshing) {
      debugPrint("⏭️ REFRESH ALREADY RUNNING - SKIPPED");
      return;
    }
    ...
    try {
      debugPrint("");
      debugPrint("================================");
      debugPrint("🔄 DRUG SEARCH REFRESH STARTED");
      debugPrint("================================");

      final data = await service.refreshAllDrugs();
      ...
      debugPrint("✅ Drug Search refreshed: ${allDrugs.length}");

      // ==========================================================
      // إعادة البحث بعد الـ Refresh
      // ==========================================================
      ...
      debugPrint("================================");
      debugPrint("✅ DRUG SEARCH REFRESH COMPLETE");
      debugPrint("================================");
      debugPrint("");
    } catch (e) {
      debugPrint("❌ ERROR REFRESHING DRUG SEARCH: $e");
    } finally {
      _refreshing = false;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: "Refresh Drugs",
            icon: const Icon(Icons.refresh_rounded, color: omanGreen),
            onPressed: loading || _refreshing ? null : refreshDrugs,
          ),
        ],
      ),
